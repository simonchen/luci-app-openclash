...
local function daip()
	...
	local daip = luci.sys.exec("uci -q get network." .. ifname .. ".ipaddr |awk -F '/' '{print $1}' 2>/dev/null |tr -d '\n'")
	if not daip or daip == "" then
		daip = luci.sys.exec("ip address show $(uci -q -p /tmp/state get network." .. ifname .. ".ifname || uci -q -p /tmp/state get network." .. ifname .. ".device) | grep -w 'inet'  2>/dev/null |grep -Eo 'inet [0-9\.]+' | awk '{print $2}' | tr -d '\n'")
	end
	if not daip or daip == "" then
